...
void CSGD_Direct3D::ChangeDisplayParam(int nWidth, int nHeight, bool bWindowed)
{
	...
	if (bWindowed)
	{
		// Setup the desired client area D3DXVECTOR2
		RECT rWindow;
		rWindow.left	= 0;
		rWindow.top		= 0;
...
//	Purpose:	Draws a rectangle of a given color to the screen.
///////////////////////////////////////////////////////////////////
void CSGD_Direct3D::DrawLine(D3DXVECTOR2 Start, D3DXVECTOR2 End, float Thickness, D3DCOLOR Color)
{
	//SLine NewLine;
	//NewLine.Points[0] = Start;
	//NewLine.Points[1] = End;
	//NewLine.Color = Color;
	//NewLine.Thickness = Thickness;
	//m_Lines.push(NewLine);
	...
	m_lpLine->SetWidth(abs(Thickness));
	m_lpLine->Begin();
	m_lpLine->Draw(Points, 2, Color);
	m_lpLine->End();
}
